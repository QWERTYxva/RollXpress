    ...
    function renderCart(cartData) {
        ...
        if (!cartData || cartData.length === 0) {
            ...
            cartData.forEach(item => {
                ...
                // 1. Determina qué nombre y precio usar, asegurando que no sean nulos
                ...
                // 2. Nos aseguramos de que el precio y la cantidad sean números válidos para el cálculo
                const precioNumerico = parseFloat(precio) || 0;
                const cantidadNumerica = parseInt(item.cantidad) || 0;
                ...
                const cartItemHTML = `
                    <div class="cart-item" data-item-id="${item.id}">
                        <div class="cart-item-info">
                            <h4>${nombre || 'Producto Desconocido'}</h4>
                            ${descriptionHTML}
                            <p class="cart-item-price">$${(precioNumerico * cantidadNumerica).toLocaleString('es-CL')}</p>
                        </div>
        ...
        const totalItems = cartData ? cartData.reduce((sum, item) => sum + (parseInt(item.cantidad) || 0), 0) : 0;
        const totalPrice = cartData ? cartData.reduce((sum, item) => {
            const esPersonalizado = parseInt(item.is_custom) === 1;
            const precio = esPersonalizado ? item.custom_precio : item.precio;
            const precioNumerico = parseFloat(precio) || 0;
            const cantidadNumerica = parseInt(item.cantidad) || 0;
            return sum + (precioNumerico * cantidadNumerica);
        }, 0) : 0;
        
        if(cartItemCount) cartItemCount.textContent = totalItems;
        if(cartTotalPrice) cartTotalPrice.textContent = `$${totalPrice.toLocaleString('es-CL')}`;
        
        // Suponiendo que la función que habilita/deshabilita el botón de checkout se llama así
